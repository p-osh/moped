...
**moped** package estimates the multivariate orthogonal polynomial based density function from sample moments (add reference of Brad's paper here and a link). Subsequently, probabilities and density of joint and marginal distribution can be computed, and resampled values from the estimated joint and marginal density can be generated.
...
Or install the **development** version of moped from [GitHub](https://github.com/) with:
...
Here we show an example demonstrating the capability of the **moped** package.
...
Data %>% 
  ...
  scale_fill_identity(
    name = NULL,
    guide = "legend",
    labels = c("Full data", "Synthetic data")
  ) 
```
