...
: Table of potential reference densities that satisfy the Sturm Louisville differential equation and can be implemented in `moped`.
...
$$
f_{\mathbf{X}}(\mathbf{x}) = f_{\nu}(\mathbf{x})\sum_{n_1 = 0}^{\infty} \dots \sum_{n_N = 0}^{\infty} C_{n_1,\dots,n_N} P_{n_1,\dots,n_N}(\mathbf{x}),
$$
...
This expression forms the central justification for the `moped` estimate, which instead is based on sample moment data has with finite polynomial order in the summation.
...
$$
\hat{f}_{\mathbf{X};\mathbf{K}}(\mathbf{x}; \{\mathbf{x}_j\}_{j=1}^M ) = f_{\nu}(\mathbf{x})\sum_{n_1 = 0}^{K_1} \dots \sum_{n_N = 0}^{K_N} \hat{C}_{n_1,\dots,n_N} P_{n_1,\dots,n_N}(\mathbf{x}),
$$

where $\hat{C}_{n_1,\dots,n_N} = (1 / M) \sum_{j=0}^M P_{n_1,\dots,n_N}(x_{1,j},\dots,x_{N,j})$.

### Estimation of the Optimal Maximum Polynomial Order
...
\qquad \text{is at a minimum.}$$

Unfortunately, the exact value of this statistic can not be determined based on sample data alone. In order to minimise this statistic, an estimation process must be implemented. But with rearrangement of the expression of $\mathcal{N}_\mathbf{K}$, we can see how an estimation procedure may be developed. Rearranging the expression of $\mathcal{N}_\mathbf{K}$ and simplifying we have,

$$
\mathcal{N}_{\mathbf{K}^{*}}\left(\{\mathbf{x}_j\}_{j=1}^M\right) + \int_{\Omega^{(N)}}\frac{f_{\mathbf{X}}(\mathbf{x})^2}{f_{\nu}(\mathbf{x})}d\mathbf{x} = \sum_{n_1=0}^{K_1}\dots\sum_{n_N=0}^{K_N}\hat{C}_{n_1,\dots,n_N}^2 + \sum_{n_1=0}^{K_1}\dots\sum_{n_N=0}^{K_N}\hat{C}_{n_1,\dots,n_N}C_{n_1,\dots,n_N},\tag{2}
$$where $\hat{C}_{n_1,\dots,n_N} = (1 / M) \sum_{j=0}^M P_{n_1,\dots,n_N}(x_{1,j},\dots,x_{N,j})$, $C_{n_1,\dots,n_N} = \mathbb{E}_\mathbf{X}\left[P_{n_1,\dots,n_N}(\mathbf{X})\right]$, and

$\int_{\Omega^{(N)}}\frac{f_{\mathbf{X}}(\mathbf{x})^2}{f_{\nu}(\mathbf{x})}d\mathbf{x}<\infty$ is constant with respect to $\mathbf{K}$.

Therefore, we can obtain the optimal maximum polynomial order $\mathbf{K}^{*}$, by minimising only the right hand side of equation (2)
